...
val device_init_def = (append_prog o process_topdecs) `
    fun device_init virtqueue virtqueue_id =
        ...
        in
            get_result buf (fn buf => Word8Array.copy buf 1 word_size (virtqueue_ptr virtqueue) 0)
        end
`;
...
val device_poll_def = (append_prog o process_topdecs) `
    fun device_poll virtqueue = let
        val word_size = Utils.get_word_size ()
        val buf = Word8Array.array (word_size + 1) (Word8.fromInt 0);
        val _ = Word8Array.copy virtqueue 0 word_size buf 1;
